                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b text-muted-foreground">
                        ...
                        {canViewAll && <th className="text-right py-3 px-4">{t('table.cost')}</th>}
                    ...
                    <tbody>
                      {records.map((r) => (
                        <tr key={r.request_id} className="border-b last:border-0 hover:bg-muted/50 cursor-pointer" onClick={() => setDetailId(r.request_id)}>
                          ...
                          {canViewAll && <td className="py-3 px-4 text-right font-mono text-xs">{formatCost(r.prompt_tokens, r.completion_tokens, r.cache_hit_input_tokens, getRecordPricing(r, modelPricing), currency, rate)}</td>}
