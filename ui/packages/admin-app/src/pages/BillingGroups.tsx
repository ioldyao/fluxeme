...
type PaymentMode = 'metered' | 'prepaid';
type BillingGroup = {
  id: string;
  name: string;
...
const modeLabel = (mode: PaymentMode): string => mode === 'prepaid' ? '预付费' : '按量计费';
const statusLabel = (status: string): string => status === 'active' ? '生效中' : '已停用';

export default function BillingGroups() {
  const queryClient = useQueryClient();
  const [name, setName] = useState('');
  const [mode, setMode] = useState<PaymentMode>('metered');
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        title="计费分组"
        description="为 API Key 配置按量计费或预付费模式。按量计费资源包优先、钱包实时结算；预付费只记录账单，由外部系统结算。"
      ...
      <Card>
        <CardContent className="space-y-4 p-5">
          ...
          <div className="grid gap-4 md:grid-cols-[1fr_240px_auto] md:items-end">
            ...
            <div className="space-y-2"><Label htmlFor="billing-group-mode">计费模式</Label><select id="billing-group-mode" className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={mode} onChange={(event) => setMode(event.target.value as PaymentMode)}><option value="metered">按量计费（余额实时扣费）</option><option value="prepaid">预付费（只记录账单/外部结算）</option></select></div>
