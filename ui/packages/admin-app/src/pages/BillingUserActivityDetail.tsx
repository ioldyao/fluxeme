...
export default function BillingUserActivityDetail({ userId, onBack }: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="grid grid-cols-6 gap-3 max-[1400px]:grid-cols-3 max-[850px]:grid-cols-2">{[['账单活动', formatCompact(summary?.activity_count ?? data?.total), '一请求一条 billing_event'], ['活跃 API Key', formatCompact(summary?.api_key_count), '活动事实中的调用维度'], ['使用模型', formatCompact(summary?.model_count), '活动事实中的模型维度'], ['总 Token', formatCompact(summary?.total_tokens), '包含免费与资源包活动'], ['资源包 units', formatCompact(summary?.package_units), '实际资源包消耗'], ['钱包实扣', formatMoney(summary?.wallet_amount), '实际钱包扣款']].map(([label, value, note]) => <div key={label} className="rounded-xl border border-border bg-card p-4 shadow-sm"><div className="text-[11px] text-muted-foreground">{label}</div><div className="mt-2 text-[22px] font-bold">{value}</div><div className="mt-1 text-[10px] text-muted-foreground">{note}</div></div>)}</section>
