...
function sourceLabel(source: string): string {
  return { package: '资源包', wallet: '钱包', package_and_wallet: '资源包 + 钱包', postpaid: '后付费', postpaid_package: '后付费 + 资源包', free_model: '免费模型', none: '无扣费', zero_cost: '零金额', unknown: '待确认' }[source] ?? source;
}
...
function ActivityRow({ activity, onClick }: { activity: BillingActivity; onClick: () => void }) {
  return (
    <tr className="cursor-pointer border-b border-border/70 hover:bg-accent/30" role="button" tabIndex={0} aria-label={`查看请求 ${activity.request_id} 的活动详情`} onClick={onClick} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onClick(); } }}>
      ...
      <td className="whitespace-nowrap px-3 py-3"><div className={`inline-flex rounded-full px-2 py-1 text-[11px] font-bold ${activity.billing_payment_mode === 'postpaid' ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700'}`}>{activity.billing_payment_mode === 'postpaid' ? '后付费' : '按量计费'}</div><div className="mt-1 text-[10px] text-muted-foreground">{activity.billing_group_name ?? '默认分组'}</div></td>
