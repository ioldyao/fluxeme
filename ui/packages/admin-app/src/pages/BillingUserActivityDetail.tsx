...
function sourceLabel(source: string): string {
  return { package: '资源包', wallet: '钱包', package_and_wallet: '资源包 + 钱包', free_model: '免费模型', none: '无扣费', zero_cost: '零金额', unknown: '待确认' }[source] ?? source;
}
...
function ActivityRow({ activity, onClick }: { activity: BillingActivity; onClick: () => void }) {
  return (
    <tr className="cursor-pointer border-b border-border/70 hover:bg-accent/30" role="button" tabIndex={0} aria-label={`查看请求 ${activity.request_id} 的活动详情`} onClick={onClick} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onClick(); } }}>
      ...
      <td className="whitespace-nowrap px-3 py-3"><span className={`inline-flex rounded-full px-2 py-1 text-[11px] font-bold ${sourceClass(activity.charge_source)}`}>{sourceLabel(activity.charge_source)}</span></td>
