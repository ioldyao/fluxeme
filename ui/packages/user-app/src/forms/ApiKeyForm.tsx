          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <select id="billingGroup" required={!isEdit} disabled={isEdit || billingGroupsLoading} className="h-10 w-full rounded-md border bg-background px-3 text-sm" value={billingGroupId} onChange={(event) => setBillingGroupId(event.target.value)}>
                ...
                {billingGroups?.map((group) => <option key={group.id} value={group.id}>{group.name} · {group.payment_mode === 'postpaid' ? '后付费' : '预付费'}</option>)}
              </select>
              {isEdit && <p className="text-xs text-muted-foreground">当前计费模式：{billingGroups?.find((group) => group.id === billingGroupId)?.payment_mode === 'postpaid' ? '后付费' : '预付费'}；如需换绑请重新创建 Key。</p>}
            </div>
            <div className="space-y-2">
              <Label>过期时间</Label>
