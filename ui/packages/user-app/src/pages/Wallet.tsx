...
import { useWalletOverview, useWalletTransactions, useRecharge, useRedeemKey, useEstimatedDays, useTokenPackageGrants } from '@fluxeme/shared/src/api/wallet';
...
export default function WalletPage() {
  ...
  return (
    <div>
      ...
      {/* ── 2. Token resource packages ── */}
      {tokenPackages && tokenPackages.length > 0 && (
        <div className="px-6 mb-8">
          <h3 className="text-sm font-semibold mb-3 flex items-center gap-2">
            <Receipt className="h-4 w-4" />
            {i18n.language.startsWith('zh') ? 'Token 资源包' : 'Token resource packages'}
          </h3>
              ...
              return (
                <div key={pkg.id} className="rounded-xl border p-5 space-y-3">
                  ...
                  <div className="flex justify-between text-xs text-muted-foreground">
                    ...
                    <span>{pkg.expires_at ? new Date(pkg.expires_at).toLocaleDateString() : 'No expiry'}</span>
                  </div>
                </div>
              );
            })}
