...
impl ProviderAdapter for DashScopeAdapter {
    async fn relay(
        ...
        super::relay_request(endpoint, path, body, "dashscope").await
    }
    ...
    async fn chat_complete(
        ...
        tracing::info!(
            ...
            "Sending request to upstream (dashscope)"
        );
        ...
        let resp = req.send().await.map_err(|e| {
            ...
            tracing::error!(
                ...
                "DashScope upstream HTTP request failed"
            );
        ...
        tracing::info!(
            ...
            "Upstream response header received (dashscope)"
        );
        ...
        tracing::info!(
            ...
            "Upstream full response received (dashscope)"
        );

        if !status.is_success() {
            let resp_text = String::from_utf8_lossy(&body_resp);
            let kind = classify_status(status.as_u16());
            tracing::error!(%status, body = %resp_text, "dashscope upstream request failed");
    ...
    async fn chat_complete_stream(
        ...
        tracing::info!(
            ...
            "Sending stream request to upstream (dashscope)"
        );
        ...
        if !status.is_success() {
            let body = response
                ...
                .unwrap_or_default();
            let kind = classify_status(status.as_u16());
            tracing::error!(%status, body = %body, "dashscope upstream stream request failed");
            return Err(ProviderError::new(
                format!("Upstream request failed with status {}", status.as_u16()),
                kind,
            ));
    ...
    async fn messages(
        ...
        tracing::info!(
            ...
            body_size = %body_size,
            timeout_ms = timeout.as_millis(),
            "Sending request to upstream (dashscope, anthropic format)"
        );
        ...
        let resp = req.send().await.map_err(|e| {
            ...
            tracing::error!(
                ...
                error_kind = ?kind,
                elapsed_ms = resp_start.elapsed().as_millis(),
                "DashScope anthropic-format upstream HTTP request failed"
            );
            ProviderError::new(format!("Request failed: {}", e), kind)
        })?;
        ...
        tracing::info!(
            ...
            ttfb_ms = resp_start.elapsed().as_millis(),
            status = status.as_u16(),
            "Upstream response header received (dashscope, anthropic format)"
        );
        ...
        tracing::info!(
            ...
            body_size = body_resp.len(),
            total_ms = resp_start.elapsed().as_millis(),
            "Upstream full response received (dashscope, anthropic format)"
        );

        if !status.is_success() {
            let resp_text = String::from_utf8_lossy(&body_resp);
            let kind = classify_status(status.as_u16());
            tracing::error!(%status, body = %resp_text, "dashscope anthropic-format request failed");
            return Err(ProviderError::new(
                format!("Upstream request failed with status {}", status.as_u16()),
                kind,
            ));
    ...
    async fn messages_stream(
        ...
        tracing::info!(
            endpoint = %endpoint.url,
            total_timeout_ms = timeout.as_millis(),
            "Sending stream request to upstream (dashscope, anthropic format)"
        );
        ...
        if !status.is_success() {
            ...
            let kind = classify_status(status.as_u16());
            tracing::error!(%status, body = %text, "dashscope anthropic-format stream request failed");
            return Err(ProviderError::new(
                format!("Upstream request failed with status {}", status.as_u16()),
                kind,
            ));
