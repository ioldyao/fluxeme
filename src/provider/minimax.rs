...
impl ProviderAdapter for MiniMaxAdapter {
    ...
    async fn messages(
        ...
        tracing::info!(
            ...
            body_size = %body_size,
            timeout_ms = timeout.as_millis(),
            "Sending request to upstream (minimax, anthropic format)"
        );
        ...
        let resp = req.send().await.map_err(|e| {
            ...
            tracing::error!(
                ...
                error_kind = ?kind,
                elapsed_ms = resp_start.elapsed().as_millis(),
                "MiniMax anthropic-format upstream HTTP request failed"
            );
            ProviderError::new(format!("Request failed: {}", e), kind)
        })?;
        ...
        tracing::info!(
            ...
            ttfb_ms = resp_start.elapsed().as_millis(),
            status = status.as_u16(),
            "Upstream response header received (minimax, anthropic format)"
        );
        ...
        tracing::info!(
            ...
            body_size = body_resp.len(),
            total_ms = resp_start.elapsed().as_millis(),
            "Upstream full response received (minimax, anthropic format)"
        );

        if !status.is_success() {
            let resp_text = String::from_utf8_lossy(&body_resp);
            let kind = classify_status(status.as_u16());
            tracing::error!(%status, body = %resp_text, "minimax anthropic-format request failed");
            return Err(ProviderError::new(
                format!("Upstream request failed with status {}", status.as_u16()),
                kind,
            ));
    ...
    async fn messages_stream(
        ...
        tracing::info!(
            endpoint = %endpoint.url,
            total_timeout_ms = timeout.as_millis(),
            "Sending stream request to upstream (minimax, anthropic format)"
        );
        ...
        if !status.is_success() {
            ...
            let kind = classify_status(status.as_u16());
            tracing::error!(%status, body = %text, "minimax anthropic-format stream request failed");
            return Err(ProviderError::new(
                format!("Upstream request failed with status {}", status.as_u16()),
                kind,
            ));
