...
import { useProbeResults } from '@shared/api/probe';
import { useUsageFunnel, useUsageAggregate, useModelActivity } from '@shared/api/usage';
import { useDashboardAggregations } from '@shared/api/dashboard';
import { fetchRoutingFlowSnapshot, useRoutingHealth } from '@shared/api/routing';
import type { Model, ModelActivity, ProbeResult } from '@shared/types';
import type { RoutingHealthChannel, RoutingHealthModel, RoutingHealthResponse } from '@shared/api/routing';
...
function useLiveTotal() {
  ...
  useEffect(() => {
    ...
    function connect() {
      ws = new WebSocket(getWsUrl('/api/health/ws'));
      ws.onopen = () => {
        setConnected(true);
        setReconnectIn(0);
        if (timer) {
