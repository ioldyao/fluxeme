...
import { Checkbox } from '@fluxeme/ui/checkbox';
import { Dialog, DialogContent, DialogHeader, DialogTitle } from '@fluxeme/ui/dialog';
import { Switch } from '@fluxeme/ui/switch';
import type { Model, ModelHealthCheckResult, Endpoint, ProbeResult } from '@fluxeme/shared/types';
...
export function ModelHealthCheckDialog({ model, open, onOpenChange, channelName, channelEndpoints }: Props) {
  ...
  const summarizeChannel = (items: ProbeResult[], endpoints: Endpoint[]) => {
    ...
    const endpointItems = items.filter((item) => item.endpoint_url);
    if (endpointItems.length === 0) {
      return {
        label: items[0].success ? `${items[0].latency_ms}ms` : '失败',
        success: items[0].success,
      };
    }
    const successCount = endpointItems.filter((item) => item.success).length;
    const totalCount = endpointItems.length;
    return {
      label: `${successCount}/${totalCount} 正常`,
